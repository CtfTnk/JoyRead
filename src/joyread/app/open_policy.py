"""Target opening contract for the staged bootstrap refactor.

This module is deliberately Qt-free. P0 records the intended decisions here;
P1-P5 will route their respective entry points through it. Keeping the policy
separate from ``LaunchIntent`` also keeps the version-1 local IPC payload stable.
"""
...
class LibraryUiContract:
    """Controls available before and after the Library runtime is ready."""
...
def library_ui_contract(state: LibraryState) -> LibraryUiContract:
    """Describe planned Library controls without creating a database runtime."""
...
def decide_open(
    ...
    *,
    ...
    """Return the target action for one entry point.

    ``READ_ONLY`` does not deny progress storage for a book already owned by a
    ready Library. For an external OS file it requires a Reader-only runtime.
    The old ``import_book_when_opening`` setting is intentionally not an input.
    """
