...
class ElidedLabel(QLabel):
    ...
    def _refresh_elision(self) -> None:
        ...
        self._sync_fixed_height(metrics)
        ...
        if self._max_lines == 1:
            ...
            display_text, is_clipped = self._multi_line_elided_text(metrics, available_width)
    ...
    def _sync_fixed_height(self, metrics: QFontMetrics) -> None:
        guard = Theme.elided_label_clip_guard if self._max_lines > 1 else 0
        self.setFixedHeight((metrics.lineSpacing() * self._max_lines) + guard)

    def _multi_line_elided_text(self, metrics: QFontMetrics, available_width: int) -> tuple[str, bool]:
        ...
        if len(line_ranges) <= self._max_lines:
            return "\n".join(source[start : start + length].strip() for start, length in line_ranges), False
